...
export const loadBlockchainData = async (gameInfo: IGameInfoType[]) => {
  ...
  if (tokenFarmData) {
    ...
    stakingBalance = await updateStakingBalance(
      tokenFarm,
      tokenAddress,
      firstAccount
    );
...
// NEW
export const getUserBalance = async (tokenFarm: any, address: string) => {
...
export const getAllUserGameList = async (
  tokenFarm: any,
  userAddress: string,
...
export const reloadBalance = async (
  ...
  if (tokenFarm) {
    ...
    try {
      const tokenFarmAddress = userData.tokenFarm._address;
      const response = await tokenFarmMethods
        .reloadBalance(address, Number(amount))
        .send({
          from: address,
        });
    } catch (err) {
...
export const withdrawBalance = async (
  ...
  if (tokenFarm) {
    ...
    try {
      const tokenFarmAddress = userData.tokenFarm._address;
      const response = await tokenFarmMethods
        .withdrawBalance(address, Number(amount))
        .send({
          from: address,
        });
    } catch (err) {
