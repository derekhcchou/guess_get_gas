...
import { answerOptions, endTimeAfterStartTime, startTimeAfterParticipationTime } from "./contentMap";
...
export const getGameInfoByIds = async (ids:string[], tokenFarm: any)=>{
  ...
  try{
    ids.forEach(async (id:string)=> {
      ...
      const participatingTime= Moment.unix(res.gameParticipateStartTime).startOf('day').format(dateFormat);
...
export const generateTime=(game:IGameInfoType)=>{
    ...
    if(ensureIsDate(game.gameParticipateStartTime)){
      const frequent = updatedGame.gameWindow.toLowerCase() as "daily"|"weekly"|"monthly"|"lifetime";
      updatedGame.gameWindowStarTime = Moment(updatedGame.gameParticipateStartTime)
        .add(startTimeAfterParticipationTime[frequent], 'days').format(dateFormat);
      updatedGame.gameWindowEndTime = Moment(updatedGame.gameParticipateStartTime).
        add(startTimeAfterParticipationTime[frequent]+endTimeAfterStartTime[frequent], 'days').format(dateFormat);
      }else{
