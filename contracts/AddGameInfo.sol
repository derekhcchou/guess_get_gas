...
contract AddGameInfo is ChainlinkClient {
        ...
        for (int256 i = 1; i <= 3; i++) {
            ...
            for (
                ...
                gameList[gameId].currencyId =
                    (int256(getRandom()) % currencyQuantity) +
                    1;
    ...
    */
    function returnGameInfo() public returns (bytes32) {
        ...
        for (int256 i = 1; i <= 3; i++) {
            ...
            for (
                ...
                if (gameList[gameId].isActivity || gameList[gameId].isClose) {
                    gameInfo[uint256(i)].gameId = gameId;
                    gameInfo[uint256(i)].gameTitle = currencyList[
                        gameList[gameId].currencyId
                    ]
                        .name;
                    gameInfo[uint256(i)].gameQuestion = questionList[
                        gameList[gameId].questionId
                    ]
                        .questionDescription;
                    gameInfo[uint256(i)].gameDescription = currencyList[
                        gameList[gameId].currencyId
                    ]
                        .currencyIntro;
                    gameInfo[uint256(i)].gameParticipateStartTime = now;
                    gameInfo[uint256(i)].gameWindow = lifeLengthList[i].title;
                    gameInfo[uint256(i)].gameProperty = gameList[gameId]
                        .property;
                    gameInfo[uint256(i)].gameLogoLink = currencyList[
                        gameList[gameId].currencyId
                    ]
                        .logoUrl;
                }
    ...
    function getAnswer(int256 _gameId) public {
        ...
        reqID = getAnswerContract.requestVolumeData(
            getAnswerContract.strAdd(
                "",
                currencyList[gameList[_gameId].currencyId].name,
                ""
            ),
